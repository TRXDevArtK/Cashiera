...
class TipeProduk extends BaseController
{
    public function index()
    {
        ...
        //
        if(isset($terms) && $terms === 'delete_type_product'){
            $post = $request->getPost();
            $data = $DeleteModel->delete_type_product($post);
            
            return $data;
        }
        
        if(isset($terms) && $terms === 'create_type_product'){
            $post = $request->getPost();
            $files = $request->getFiles();
            $data = $CreateModel->create_type_product($post, $files);
            
            return $data;
        }
        
        if(isset($terms) && $terms === 'update_type_product'){
            $post = $request->getPost();
            $files = $request->getFiles();
            $data = $UpdateModel->update_type_product($post, $files);
        ...
        return view('admin/pages/tipe_produk', $data);
    }
}